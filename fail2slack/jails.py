...
def get_jails_status(active_jails):
    ...
    try:
        for jail in active_jails:
            jails_status = subprocess.check_output("fail2ban-client status " + jail, shell=True).decode('utf-8')
            ...
            current_failed = re.search(r"Currently failed:(.*\b)", jails_status, re.IGNORECASE | re.MULTILINE)
            total_failed = re.search(r"Total failed:(.*\b)", jails_status, re.IGNORECASE | re.MULTILINE)
            current_banned = re.search(r"Currently banned:(.*\b)", jails_status, re.IGNORECASE | re.MULTILINE)
            total_banned = re.search(r"Total banned:(.*\b)", jails_status, re.IGNORECASE | re.MULTILINE)
